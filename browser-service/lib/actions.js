...
async function navigate(page, params) {
  ...
  if (wait_for_selector) {
    await page.waitForSelector(wait_for_selector, { timeout: 10000 }).catch(() => null);
  }
  if (wait_ms) {
...
async function evaluate(page, params) {
  ...
  if (script.length > SAFE_LIMIT) throw new Error(`Script too long (max ${SAFE_LIMIT} chars)`);
...
async function getLinks(page, params) {
  const { filter } = params;
  const links = await page.$$eval('a[href]', (els, f) => {
    return els
      .map(el => ({ text: el.innerText.trim(), href: el.href }))
      .filter(l => l.href && !l.href.startsWith('javascript:'))
      .filter(l => !f || l.text.toLowerCase().includes(f.toLowerCase()) || l.href.includes(f))
      .slice(0, 200);
  }, filter || '');
